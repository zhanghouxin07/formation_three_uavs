...
geometry_msgs::PoseStamped local_position, drone2_local_position;
...
void drone2_local_position_cb(const geometry_msgs::PoseStamped::ConstPtr& msg){
    drone2_local_position = *msg;
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber drone2_local_position_sub = nh.subscribe<geometry_msgs::PoseStamped>
			("/drone2/mavros/setpoint_position/local",10,drone2_local_position_cb);
	...
	drone2_local_position.pose.position.x=0;
	drone2_local_position.pose.position.y=0;
	drone2_local_position.pose.position.z=2.5;
    while(ros::ok()){
        auto st=clock();
		...
		local_pos_pub.publish(pose);
		pose.pose.position.x = drone2_local_position.pose.position.x;
		pose.pose.position.y = drone2_local_position.pose.position.y;
		pose.pose.position.z = drone2_local_position.pose.position.z;
		ros::spinOnce();
		rate.sleep();
    }
