...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()){
		...
		// judge time sync
		if(sec_cnt==0) 
		if(uav0_cnt[0]==main_cnt && uav2_cnt[0]==main_cnt) sec_cnt=1;
		...
		cout<<uav0_p_cnt<<' '<<uav2_p_cnt<<' '<<uav0_t_cnt<<' '<<uav2_t_cnt<<endl;
		// judge pos and tar sync
		int mysolve[2];
		if(sec_cnt==1)
		if(uav0_p_cnt==main_cnt && uav2_p_cnt==main_cnt 
				&& uav0_t_cnt==main_cnt && uav2_t_cnt==main_cnt){
		/*
		* 	solve with all information
		*/
			int * _mysolve = multiUAV1.solve(multiUAV1.pos,multiUAV1.tar,N);
		...
		if(sec_cnt==2)
		if(uav0_swfg_cnt==main_cnt && uav2_swfg_cnt==main_cnt){
			...
			tmp_destination_x=multiUAV1.pos[0][0]-le.x;
			tmp_destination_y=multiUAV1.pos[0][1]-le.y;
			sec_cnt=3;
		}
